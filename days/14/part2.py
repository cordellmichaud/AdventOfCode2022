...
def draw_map(rock_group: RockGroup, sands: list[Point], 
    ...
    for y in range(bbox[0].y, bbox[1].y + 1):
        ...
        for x in range(bbox[0].x, bbox[1].x + 1):
            if (x, y) == (500, 0) and Point(500, 0) not in sands:
                line += '+'
            elif y == bbox[1].y or rock_group.collides(Point(x, y)):
                line += '#'
...
def parse_rock_file(file_path: str | Path) \
    ...
    y_bounds: Optional[list[int]] = [0, 2]
...
def simulate_sand(source_pos: Point, rock_group: RockGroup, 
    ...
    while True:
        ...
        if down_pos.y == bbox[1].y:
            return current_pos
        
        down_collides = rock_group.collides(down_pos) \
            or down_pos in sands
        dleft_collides = rock_group.collides(dleft_pos) \
            or dleft_pos in sands
        dright_collides = rock_group.collides(dright_pos) \
            or dright_pos in sands
...
def main():
    ...
    while True:
        ...
        sands[sand_pos] = 1
        sand_count += 1
        if sand_pos.x < bbox[0].x:
            bbox = (Point(sand_pos.x, bbox[0].y), bbox[1])
        if sand_pos.x > bbox[1].x:
            bbox = (bbox[0], Point(sand_pos.x, bbox[1].y))
